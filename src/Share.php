...
namespace Alfthegreatest\Share;
...
class Share
{
    ...
    public function x()
    {
        if (is_null($this->title)) {
            $this->title = config('laravel-share.services.x.text');
        }

        $base = config('laravel-share.services.x.uri');
        $url = $base . '?text=' . urlencode($this->title) . '&url=' . $this->url;

        $this->buildLink('x', $url);
